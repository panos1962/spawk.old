...
BEGIN {
	...
	FS = ","
	# No need to initialize variables, but is no harm to do so.
	# Anyway, the following lines may be deleted.
	pairID = 0
	dateID = 0
	cpairStr = ""
	cdateStr = ""
...
function GetValue(x,				ndx, val, st)
{
	...
	val = val + int(substr(x, st, 6 - ndx))
	return(val)
}

function GetTimeVal(timeStr,				hms)
{
...

{
	...
	if (ipairStr != cpairStr) {
		...
		if (spawk_first(data))
			...
			# If pair is not found then insert in to
			# `pair' table.
			if (spawk_update("INSERT INTO `pair` (`Pair`) " \
				"VALUES ('" ipairStr "')")) {
				spawk_command("ROLLBACK WORK", 2)
				print "insert `pair` failed " \
					"(`Pair` = '" ipairStr "') (" \
	...
	if (dateStr != cdateStr) {
		...
		{
			# If date is not found then insert in to
			# `date` table.
			if (spawk_update("INSERT INTO date " \
				"(`DateStr`) VALUES ('" dateStr "')")) {
				spawk_command("ROLLBACK WORK", 2)
				print "insert `date` failed " \
					"(`DateStr` = '" ipairStr \
					"') (" spawk_error() \
					")" > "/dev/stderr"
				next
